...
export default function Page() {
  ...
  // const products = [
  //   { id: 1, 'fuel-type': "Gasoline/Petrol"},
  //   { id: 2, 'fuel-type': "Diesel" },
  //   { id: 3, 'fuel-type': "Hybrid" },
  //   { id: 4, 'fuel-type': "Electric" },
  // ];

  const headers = [
    { label: "ID", className: "px-6 py-3" },
    { label: "Fuel_type", className: "px-6 py-6" },
  ...
  const handleEdit = async () => {
    try {
      const response = await fetch(
        `https://bmexports.elitecoderstestlink.com/api/update-ft/${editId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        toast.success("Fuel type has been edit");
        fetchFuel();
        onOpenChange();
      }
    } catch (err: any) {
  ...
  const handleDelete = async (id: any) => {
    ...
    try {
      const response = await fetch(
        `https://bmexports.elitecoderstestlink.com/api/delete-ft/${id}`,
        {
          method: "DELETE",
        },
      );
